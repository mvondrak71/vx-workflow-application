...
/*
 * Copyright (c) 2009-2010, 2013, 2015 Wind River Systems, Inc. 
 *
...
--------------------
29dec15,clb  Fix rtpSmpSigkillTest and rtpSmpSigstopSigcontTest failure
27jan15,pad  Adapted for CERT.
05sep13,jdw  Fix Coverity issues (WIND00433558)
28jul10,pad  completed LP64 adaptation: use TASK_ID type and TASK_ID_ERROR
	     macro where necessary.
03feb10,jpb  Updated for LP64 adaptation.  Replaced sleep()
             with taskDelay() to remove dependency on pthreadLib.
31aug09,kk   clean up documentation
12may09,h_l  adjust philosopher and fork index in log message.
22apr09,h_l  add CPU index info in log message.
21jan09,pad  written.
*/
...
int main
    ...
    if (argv[1] != NULL)
        ...
        if (!strncmp (argv[1], "-d", 2))
            {
            demoDuration = atoi (argv[2]);
            }
        if (!strncmp (argv[1], "-n", 2))
            {
            philosAttr.numPhil = atoi (argv[2]);
            }
    ...
    if ( philosAttr.syncSemId == NULL )
        {
        printf ("Cannot create sync semaphore.\n");
        exit (1);
        }
    ...
    if ( forksAttr.forks == NULL )
        {
        printf ("Cannot create forks.\n");
        exit (1);
        }
    ...
    (void) semFlush (philosAttr.syncSemId);
    ...
    (void) semDelete (philosAttr.syncSemId);
    free (philosAttr.pPhil);
    free (philosAttr.iAmReady);
    free (stats.foodIntake);
    free (stats.goHungry);
    (void) semDelete (forksAttr.forkMutex);
    free (forksAttr.forks);
    (void) semDelete (semOutId);
    semOutId = NULL;
...
void philosopherRun
    ...
    (void) semTake (pPhilsAttr->syncSemId, WAIT_FOREVER);
...
BOOL forksGet
    ...
    (void) semTake (pForksAttr->forkMutex, WAIT_FOREVER);
